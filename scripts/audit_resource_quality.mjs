...
if (!evidencePolicy?.display || typeof evidencePolicy.display !== 'object' || Array.isArray(evidencePolicy.display)) {
  ...
  for (const scope of ['xx_reproductive', 'xy_reproductive', 'x_linked', 'y_linked', 'menstrual_cycle_context', 'ovarian_context', 'uterine_context', 'androgen_reproductive_context', 'all_bodies_preconception_fertility_context', 'all_bodies_hormone_therapy_context', 'all_bodies_pregnancy_lactation_context']) {
    if (typeof evidencePolicy.display.scope_labels?.[scope] !== 'string') {
      errors.push(`evidence_policy.json: display scope label ${scope} is missing`);
    }
  }
}
...
if (runtimeSummary?.cycle_support_layer?.cycle_day_comparison_min_observations
  !== cycleSupport?.review_schema?.minimum_observations_for_cycle_day_comparison) {
  errors.push(`runtime_validation_summary.json: cycle_support_layer.cycle_day_comparison_min_observations is ${runtimeSummary?.cycle_support_layer?.cycle_day_comparison_min_observations ?? '(missing)'}, expected ${cycleSupport?.review_schema?.minimum_observations_for_cycle_day_comparison ?? '(missing)'}`);
}
