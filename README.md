...
* **Plain-English coverage:** Marker translations are authored in `layperson_translations.json` rather than embedded in TypeScript. Dedicated explanations are used where available; the current resource covers 171/580 curated standard rsIDs, including high-value PGx star-allele, haplotype, proxy, cancer-confirmation, low-effect PRS, and clinical-boundary explanations. Otherwise the UI derives a conservative presentation from the marker's own resource-authored impact, interpretation, and raw-DNA limitation without changing evidence, claim boundaries, or actionability.
