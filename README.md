...
* **Cycle-support layer:** When the person selects a relevant context, reports expose structured timing logs, PMDD-like symptom tracking, exact contraceptive ingredient prompts, exogenous hormone product/route/dose/monitoring questions, symptom-day support, heavy-bleeding/pelvic-pain escalation, adenomyosis workup boundaries, or androgen/prostate/testicular follow-up. With no selection, context-specific guidance and medication-composition warnings remain hidden unless an explicitly supplied medication name makes a label guardrail relevant. The layer does not infer current estrogen/progesterone, hormone regimen, PMDD, adenomyosis, or medication response from DNA.
