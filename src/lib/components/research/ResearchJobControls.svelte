...
<script lang="ts">
  ...
  let statusText = $derived(
    isCancelling
      ? "Cancelling"
      : isStarting && (!job || job.status === "idle")
        ? "Starting"
        : jobStatusText(job),
  );
  ...
  let qdrantSampleCount = $derived.by(() => {
    ...
    return liveProgress.qdrantSampleCount ?? job?.qdrant_sample_count ?? null;
  });

  let showProgress = $derived(
    isStarting || (job != null && job.status !== "idle"),
  );
</script>
...
<div class="glass-card research-job-card">
  ...
  {#if showProgress}
    {#if isStarting && (!job || job.status === "idle")}
      <div class="starting-panel" role="status" aria-live="polite">
        <div class="starting-pulse"></div>
        <div class="starting-copy">
          <strong>Starting sweep</strong>
          <span>
            Queuing markers
            {#if scopePreview?.total_unique}
              ({scopePreview.total_unique.toLocaleString()} in scope)
            {/if}
            and checking Qdrant cache…
          </span>
          {#if pipelineTuning}
            <span class="starting-tuning">
              Adaptive batch {pipelineTuning.enrich_batch_size} · {pipelineTuning.prepare_concurrency} parallel prepare
            </span>
          {/if}
        </div>
      </div>
