...
* **Concise sex display:** The report, profile list, and advanced profile summary show `Male` or `Female` when normalized X/Y call coverage supports that chromosome pattern, and retain `Unknown` or `Uncertain` when the data is insufficient or conflicting. Legacy verbose labels are shortened on primary surfaces; Ancestry’s numeric chromosome aliases (`23/24/25`) are normalized during import, refresh, and fixture auditing. The supporting limitation is available from the adjacent help tooltip rather than repeated in the primary result.
