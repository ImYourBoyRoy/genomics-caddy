...
* Offline status now includes a machine-readable `remote_check` result. An incomplete or timed-out remote probe cannot be presented as `Current` or as an actionable update; probe tasks are cancelled with the check so late network responses cannot reintroduce stale update flags. The MCP audit reports only aggregate checked/failed/timeout counts, and the desktop UI uses one concise retry message.
