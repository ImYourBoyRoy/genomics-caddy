...
pub fn evidence_card_from_hit(hit: &QdrantHit, point_id: Option<u64>, explanation: Option<MatchExplanation>) -> EvidenceCard {
...
pub fn evidence_card_from_payload(
    ...
    EvidenceCard {
        ...
        position_grch38: p.get("position_grch38").or_else(|| p.get("position")).and_then(|v| v.as_i64()),
        ...
        has_direction,
        p_value_min: p["p_value_min"].as_f64().or_else(|| p["gwas_best_pvalue"].as_f64()),
        ...
        clinical_actionability_score: p["clinical_actionability_score"].as_f64().unwrap_or(0.0) as f32,
        wellness_actionability_score: p["wellness_actionability_score"].as_f64().unwrap_or(0.0) as f32,
        ...
        missing_field_count: p["missing_field_count"].as_u64().unwrap_or(missing_fields.len() as u64) as u32,
        quality_flags,
        missing_fields,
        ...
        qdrant_point_id: point_id.map(|id| id.to_string()).or_else(|| {
            Some(string_to_u64(&format!(
                "{}_{}_{}",
                p["sample_id"].as_i64().unwrap_or(0),
                rsid,
                opt_str(p, "genotype").unwrap_or_default().split('/').next().unwrap_or("")
            ))
            .to_string())
        }),
    }
