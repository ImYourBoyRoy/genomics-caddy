...
pub fn migrate_evidence_schema(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        ...
        CREATE INDEX IF NOT EXISTS idx_source_records_rsid ON source_records(rsid);
        CREATE INDEX IF NOT EXISTS idx_source_records_source ON source_records(source_name, rsid);
        ...
        CREATE INDEX IF NOT EXISTS idx_api_cache_key ON api_cache_entries(normalized_cache_key);
        CREATE INDEX IF NOT EXISTS idx_api_cache_expires ON api_cache_entries(expires_at);
