...
pub fn resolve_strand_orientation(
    ...
    if user_alleles.is_empty() || expected_plus_alleles.is_empty() {
        return ("no_data".to_string(), user_alleles.to_vec(), "not_required".to_string());
    }

    // Convert everything to uppercase
    let user: Vec<String> = user_alleles.iter().map(|s| s.to_uppercase()).collect();
    let expected: Vec<String> = expected_plus_alleles.iter().map(|s| s.to_uppercase()).collect();
    ...
    if direct_match {
        // Direct match!
        // Is it a palindrome?
        let is_palindrome = (expected.contains(&"A".to_string()) && expected.contains(&"T".to_string()))
    ...
    // Conflicting alleles
    ("orientation_unverified".to_string(), user, "conflicting".to_string())
}
...
pub async fn get_variant_evidence_core(
    ...
    if let Some((g, sig, cond, rel)) = cached_ref {
        ...
        let search_res = fetch_api_cached(db_path, &search_url, api_key, 86400 * 3).await.ok();
        ...
        if let Some(search_val) = search_res
            && let Some(ids) = search_val["esearchresult"]["idlist"].as_array()
                && !ids.is_empty() {
                    ...
                    let summary_url = format!("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=clinvar&id={}&retmode=json", id_str);
                    if let Ok(summary_res) = fetch_api_cached(db_path, &summary_url, api_key, 86400 * 3).await
                        && let Some(summary) = summary_res["result"][&id_str].as_object() {
                            clinvar_sig = summary.get("clinical_significance").and_then(|v| v.get("description")).and_then(|v| v.as_str()).map(|s| s.to_string());
                            clinvar_review = summary.get("clinical_significance").and_then(|v| v.get("review_status")).and_then(|v| v.as_str()).map(|s| s.to_string());
                            
                            if let Some(traits) = summary.get("trait_set").and_then(|v| v.as_array()) {
                                let trait_names: Vec<String> = traits.iter().filter_map(|t| t.get("trait_name").and_then(|v| v.as_str()).map(|s| s.to_string())).collect();
        ...
        clinvar_relevant_allele = clinvar_relevant_allele.or(catalog_risk_allele).map(|s| s.to_uppercase());
    ...
    let ensembl_url = format!("https://rest.ensembl.org/variation/homo_sapiens/{}?content-type=application/json", clean_rsid);
        ...
        && let Some(mappings) = ensembl_res["mappings"].as_array()
            && !mappings.is_empty()
                && let Some(allele_str) = mappings[0]["allele_string"].as_str() {
                    expected_plus_alleles = allele_str.split('/').map(|s| s.to_string().to_uppercase()).collect();
                }
    ...
    let lower_gene = resolved_gene.as_ref().map(|s| s.to_lowercase()).unwrap_or_default();
    if lower_gene == "dpyd" || lower_gene == "brca1" || lower_gene == "brca2" || lower_gene == "cyp2c19" || lower_gene == "cyp2d6" || lower_gene == "cyp2c9" || lower_gene == "slco1b1" {
        is_high_stakes = true;
    }
    ...
    if !is_missing {
        ...
        if is_high_stakes && (orientation_status == "ambiguous" || orientation_status == "conflicting") {
...
pub fn append_discovered_findings_to_path(
    ...
    for f in new_findings {
        if !list.iter().any(|existing| existing.rsid.to_uppercase() == f.rsid.to_uppercase()) {
            list.push(f);
        }
    }
...
pub fn get_db_discovered_findings(
    ...
    let rows = stmt.query_map(params![sample_id], |row| {
        ...
        let user_alleles = user_genotype.as_ref()
...
fn detect_dosing_phrase(text: &str) -> Option<(String, String)> {
    ...
    let words: Vec<&str> = text_lower.split(|c: char| !c.is_alphanumeric() && c != '.').collect();
    let units = vec!["mg", "mcg", "ug", "iu", "g", "milligrams", "micrograms", "grams", "units"];
    ...
    for (idx, &word) in words.iter().enumerate() {
        ...
        for &unit in &units {
            if word.ends_with(unit) && word.len() > unit.len() {
                ...
                if num_part.chars().any(|c| c.is_ascii_digit()) && num_part.chars().all(|c| c.is_ascii_digit() || c == '.') {
                    return Some((word.to_string(), format!("Dosing unit '{}' detected directly with quantity", unit)));
                }
            }
        }
        
        // Case 2: Word is a number and the next word is a unit (e.g. "500 mg")
        if word.chars().any(|c| c.is_ascii_digit()) && word.chars().all(|c| c.is_ascii_digit() || c == '.')
            && idx + 1 < words.len() {
                let next_word = words[idx + 1];
                if units.contains(&next_word) {
                    return Some((format!("{} {}", word, next_word), format!("Dosing unit '{}' detected after quantity", next_word)));
                }
            }
    }
    None
...
pub fn run_deterministic_safety_checks(
    ...
    let diagnosis_phrases = vec![
        ("you have", "Diagnosis Claim: Avoid absolute 'you have' statements without citations and safety disclaimers."),
        ("you suffer from", "Diagnosis Claim: Avoid declaring 'you suffer from' a disease without citations and safety disclaimers."),
        ("you are diagnosed with", "Diagnosis Claim: Avoid declaring 'you are diagnosed with' a disease without citations and safety disclaimers."),
        ("diagnose you", "Diagnosis Claim: Decline to diagnose the user without citations and safety disclaimers."),
        ("diagnose the", "Diagnosis Claim: Avoid making diagnostic assertions without citations and safety disclaimers."),
        ("your diagnosis", "Diagnosis Claim: Avoid referencing the user's diagnosis without citations and safety disclaimers."),
    ];
    ...
    for (phrase, msg) in diagnosis_phrases {
        if text_lower.contains(phrase) {
            ...
            if !citations_present && !safety_caveat_present {
                severity = "fail";
                issue_message = format!("{}. Missing both source citations and medical validation caveat.", msg);
            } else if !citations_present {
                severity = "fail";
                issue_message = format!("{}. Must provide explicit source citations (PMID, ClinVar, etc.) to validate this diagnosis.", msg);
            } else if !safety_caveat_present {
                severity = "fail";
                issue_message = format!("{}. Must include the warning caveat that following a model without validating with a medical professional could cause serious harm or death.", msg);
            }
    ...
    let treatment_phrases = vec![
        ("you should take", "Treatment Recommendation: Avoid recommending compounds without citations and safety disclaimers."),
        ("start taking", "Treatment Recommendation: Avoid telling the user to start taking compounds without citations and safety disclaimers."),
        ("stop taking", "Treatment Recommendation: Avoid telling the user to stop taking compounds without citations and safety disclaimers."),
        ("increase dose", "Dosage Recommendation: Avoid dosing adjustments without citations and safety disclaimers."),
        ("decrease dose", "Dosage Recommendation: Avoid dosing adjustments without citations and safety disclaimers."),
        ("prescribed", "Treatment Recommendation: Avoid referring to prescription recommendations without citations and safety disclaimers."),
        ("daily dose", "Dosage Recommendation: Avoid specifying daily dosage thresholds without citations and safety disclaimers."),
        ("recommended dose", "Dosage Recommendation: Avoid specifying recommended dose values without citations and safety disclaimers."),
    ];
    ...
    for (phrase, msg) in treatment_phrases {
        if text_lower.contains(phrase) {
            ...
            if !citations_present && !safety_caveat_present {
                ...
                severity = "fail";
                issue_message = format!("{}. Must provide explicit source citations (PMID, ClinVar, etc.) to validate this recommendation.", msg);
            } else if !safety_caveat_present {
                severity = "fail";
                issue_message = format!("{}. Must include the warning caveat that following a model without validating with a medical professional could cause serious harm or death.", msg);
            }
    ...
    if let Some((phrase_match, _detail)) = detect_dosing_phrase(report_text) {
        ...
        if !citations_present && !safety_caveat_present {
            severity = "fail";
            issue_message = format!("Dosing Unit Detected ('{}'): Missing both source citations and medical validation caveat.", phrase_match);
        } else if !citations_present {
            severity = "fail";
            issue_message = format!("Dosing Unit Detected ('{}'): Must provide explicit source citations (PMID, ClinVar, etc.) to validate this dosing.", phrase_match);
        } else if !safety_caveat_present {
            severity = "fail";
            issue_message = format!("Dosing Unit Detected ('{}'): Must include the warning caveat that following a model without validating with a medical professional could cause serious harm or death.", phrase_match);
        }
    ...
    let absolute_assertions = vec![
        ("guarantee", "Assertion Warning: Avoid absolute genetic guarantees."),
        ("prevent", "Assertion Warning: Avoid absolute prevention claims."),
        ("cure", "Assertion Warning: Avoid declaring genetic cures."),
        ("will develop", "Assertion Warning: Avoid absolute predictions like 'will develop'."),
    ];
    ...
    for v in variant_evidence_list {
        if !v.safe_for_ai_context
            && text_lower.contains(&v.rsid.to_lowercase()) {
                findings.push(SafetyFinding {
    ...
    let has_active_clinical = variant_evidence_list.iter().any(|v| v.interpretation_status == "active_clinical");
    if has_active_clinical {
        let has_disclaimer = text_lower.contains("disclaimer") || text_lower.contains("medical advice") || text_lower.contains("educational purposes");
        if !has_disclaimer {
            findings.push(SafetyFinding {
