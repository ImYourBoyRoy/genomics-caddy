...
* **Concise sex display:** The report and profile list show `Male` or `Female` when X/Y call coverage supports that chromosome pattern, and retain `Unknown` or `Uncertain` when the data is insufficient. The supporting limitation is available from the adjacent help tooltip rather than repeated in the primary result.
...
* Dashboard guidance uses compact visible prompts; detailed context boundaries remain available through the existing safety disclosure or accessible help tooltip.
