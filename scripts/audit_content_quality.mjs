...
if (jsonOutput) {
  ...
  const interpretationReview = summary.copy.repeated_interpretation_review;
  console.log(`Repeated interpretations: marker-family context=${interpretationReview.classifications.marker_family_context.values}; section-shared context=${interpretationReview.classifications.section_shared_context.values}; cross-section reuse=${interpretationReview.classifications.cross_section_reuse.values}.`);
  console.log(`Copy reuse classes: shared boundaries=${summary.copy.reuse_classes.shared_boundary.repeated_values}; shared follow-ups=${summary.copy.reuse_classes.shared_follow_up.repeated_values}; marker interpretations=${summary.copy.reuse_classes.repeated_marker_interpretation.repeated_values}; marker impact=${summary.copy.reuse_classes.marker_impact.repeated_values}; generic fallback rows=${summary.copy.reuse_classes.fallback_copy.candidate_entries}.`);
  console.log(`Simple meaning: authored=${summary.plain_meaning.authored}; identifier templates=${summary.plain_meaning.templated}; fallback=${summary.plain_meaning.fallback}; missing=${summary.plain_meaning.missing}; translated standard rsIDs=${summary.plain_meaning.translated_standard_rsids}.`);
  ...
  for (const pack of summary.pack_summaries) {
    console.log(`  ${pack.id}: rows=${pack.marker_rows} unique_rsids=${pack.unique_standard_rsids} duplicate_rows=${pack.duplicate_standard_rows} authored_meaning=${pack.plain_meaning.authored} templated_meaning=${pack.plain_meaning.templated} fallback_meaning=${pack.plain_meaning.fallback} missing_meaning=${pack.plain_meaning.missing} missing_action=${pack.findings_without_action}`);
  }
