...
* **`npm run audit:tauri-ui`**: Privacy-safe desktop QA against the running Tauri window through the loopback UI bridge. Exercises Simple, Clinical, and Compare modes, checks concise sex labeling, desktop overflow, two-column finding layout, clinical-table separation, and redundant public copy; requires `npm run tauri:dev` to be running and never prints genotype data.
