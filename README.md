...
* **`npm run audit:tauri-update`**: Starts a disposable Tauri fixture with a synthetic sample and a stale liftover identity, then verifies desktop download, registry update, authoritative post-sync status, update-copy clearing, and report readiness. It removes only the temporary fixture and never touches configured app data or prints genotype data.
