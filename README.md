...
* Offline update badges derive from the named locally-installed update list, hide the previous probe's flags while an authoritative status check is running or has failed, clear a successfully synced asset while the post-sync probe runs, and describe remaining items as newer remote versions so downloaded/current resources are not conflated. The GWAS sync path also honors a proven newer remote identity before deciding that an installed catalog is current.
