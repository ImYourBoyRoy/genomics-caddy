...
* **Plain-English coverage:** Marker translations are authored in `layperson_translations.json` rather than embedded in TypeScript. Dedicated explanations are used where available; otherwise the UI derives a conservative presentation from the marker's own resource-authored impact, interpretation, and raw-DNA limitation without changing evidence, claim boundaries, or actionability.
